    ...
    T: Plugin,
{
    ...
    fn enter_interface_type(&self, node: &InterfaceType, context: &mut SchemaVistorContext) {
        match self.plugin.interface_type(node) {
            Some(node) => {
                context.nodes.push(node);
            }
            None => { /* @todo */ }
    ...
    fn enter_union_type(&self, node: &UnionType, context: &mut SchemaVistorContext) {
        match self.plugin.union_type(node) {
            Some(node) => {
                context.nodes.push(node);
            }
            None => { /* @todo */ }
